...
contract VaultControllerTest is Test {
    ...
    function test__initilization() public {
        ...
        assertEq(
            address(controller.permissionRegistry()),
    ...
    function test__addStakingRewardsTokens() public {
        ...
        // RewardsInfo
        assertEq(
            IMultiRewardStaking(staking).rewardInfos(iRewardToken2).ONE,
            1 ether
        );
        assertEq(
            IMultiRewardStaking(staking)
                .rewardInfos(iRewardToken2)
                .rewardsPerSecond,
            0.1 ether
        );
        assertEq(
        ...
        assertEq(
            uint256(
                IMultiRewardStaking(staking).rewardInfos(iRewardToken2).index
            ),
            1 ether
        );
        assertEq(
        ...
        assertEq(
            uint256(
                IMultiRewardStaking(staking).escrowInfos(iRewardToken2).offset
            ),
            1 days
        );
    }
    ...
    function test__changeStakingRewardsSpeeds() public {
        ...
        controller.changeStakingRewardsSpeeds(
            targets,
            rewardTokens,
            rewardSpeeds
        );
    ...
    function testFail__addTemplateCategories_templateCategory_already_exists()
        public
    {
        bytes32[] memory templateCategories = new bytes32[](1);
        templateCategories[0] = templateCategory;
    ...
    function test__toggleTemplateEndorsements() public {
        ...
        vm.expectEmit(true, true, true, false, address(templateRegistry));
        emit TemplateEndorsementToggled(
            templateCategory,
            templateId,
            false,
            true
        ...
        Template memory template = templateRegistry.getTemplate(
            templateCategory,
            templateId
        );
    ...
    function testFail__toggleTemplateEndorsements_templateId_doesnt_exist()
        public
    {
        bytes32[] memory templateCategories = new bytes32[](1);
        templateCategories[0] = templateCategory;
    ...
    function test__setDeploymentController() public {
        IDeploymentController newDeploymentController = IDeploymentController(
            address(
                new DeploymentController(
                    address(this),
                    factory,
                    ICloneRegistry(address(1)),
                    ITemplateRegistry(address(2))
                )
            )
        );
        ...
        assertEq(
            address(controller.deploymentController()),
            address(newDeploymentController)
        );
    ...
    function testFail__setDeploymentController_old_address() public {
        controller.setDeploymentController(
            IDeploymentController(address(deploymentController))
        );
    }

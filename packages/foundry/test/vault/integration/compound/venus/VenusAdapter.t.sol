...
contract VenusAdapterTest is AbstractAdapterTest {
  ...
  function setUp() public {
    uint256 forkId = vm.createSelectFork(vm.rpcUrl("bsc"));
    vm.selectFork(forkId);

    testConfigStorage = ITestConfigStorage(address(new VenusTestConfigStorage()));
  ...
  function increasePricePerShare(uint256 amount) public override {
    deal(address(asset), address(vaiToken), asset.balanceOf(address(vaiToken)) + amount);
  }
  ...
  function verify_adapterInit() public override {
    ...
    assertEq(asset.allowance(address(adapter), address(vaiToken)), type(uint256).max, "allowance");
  }
  ...
  function test__RT_mint_withdraw() public override {
    _mintAssetAndApproveForAdapter(adapter.previewMint(vaiDefaultAmount), bob);

...
contract VelodromeAdapterTest is AbstractAdapterTest {
  ...
  function setUp() public {
    ...
    testConfigStorage = ITestConfigStorage(address(new VelodromeTestConfigStorage()));
  ...
  function _setUpTest(bytes memory testConfig) internal {
    ...
    address velo = gauge.rewards(2);
    asset = IERC20(gauge.stake());

    setUpBaseTest(IERC20(asset), address(new VelodromeAdapter()), address(gauge), 10, "Velodrome", true);

...
export const useButterHoldings: Pop.Hook<BigNumberWithFormatted> = ({ chainId, address, account }) => {
  const [butter, butterBatch] = useNamedAccounts(chainId as any, ["butter", "butterBatch"]);

  const { data: butterBatches } = useButterBatches({ account, chainId, address: butterBatch?.address });
  const { data: butterPrice } = usePrice({ address: butter?.address, account, chainId });
  const { data: butterBalance } = useBalanceOf({ address: butter?.address, account, chainId });
  if (butterBatches?.claimableMintBatches) {
    const claimableBalance = getClaimableBalance(butterBatches?.claimableMintBatches);
    const holdingValue = getHoldingValue(
      (butterBalance?.value || constants.Zero).add(claimableBalance),

...
const ListItem: FC<ListItemProps> = ({ children, value, onClick }) => {
  ...
  return (
    <li
      value={value}
      className={`my-1 bg-transparent border-3 text-base md:text-lg hover:bg-customPaleGray hover:bg-opacity-40 rounded-lg ${
        selected === value ? "border-primary" : "border-transparent"
      }`}
    >
      <button
